...
int main(int argc, const char *argv[]) try
{
    ...
    const char * const usage_msg =
        "USAGE: \n"
        "\n"
        "generate_robroute -rnd NS EW p          out_filename r c0 c1 c2\n"
        "  --random parking lot of given dimensions\n"
        "    with edge (no-wall) probability p;\n"
    ...
    if (string(argv[1]) == "-rnd") { // random grid
        ...
        const double         p   = std::atof(argv[4]);
        std::printf("Searching connected(!) random grid with NS %d, EW %d, and edge probability %f",NS,EW,p);
        do {
    ...
    GridSpace::write_robroute(file,statvec);
    file.close();
